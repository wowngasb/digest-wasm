var unbzip2 = {};
unbzip2.decode = function(bytes){
  return unbzip2.simple(unbzip2.array(bytes));
};

unbzip2.decodeU8 = function(bytes, out){
  var bits = unbzip2.array(bytes);
  var size = unbzip2.header(bits);
  var chunk = 0;
  do{
    chunk = unbzip2.decompress(bits, size, 0, out);
  }while(chunk != -1);
  return out;
};

unbzip2.array = function(bytes){
  var bit = 0, byte = 0;
...
unbzip2.simple = function(bits){
  var size = unbzip2.header(bits);
  var all = '', chunk = '';
  do{
    all += chunk;
    chunk = unbzip2.decompress(bits, size);
  }while(chunk != -1);
  return all;
}

unbzip2.header = function(bits){
...
unbzip2.decompress = function(bits, size, len, out){
...
if (typeof window === "object") {
    window.unbzip2 = unbzip2;
}
if (typeof module !== "undefined") {
    module.exports = unbzip2;
}
